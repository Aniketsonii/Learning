...
void getPage(int logical_address)
{
    ...
    fprintf(output_text,"Virtual address: %d Physical address: %d Value: %d\n", logical_address, (frameNumber << 8) | offset, value);
}
...
void insertIntoTLB(int pageNumber, int frameNumber)
{
    ...
    if (i == numberOfTLBEntries)
    {
        ...
        {                                                   // and the TLB still has room in it
...
int main(int argc, char *argv[])
{
    ...
    fprintf(output_text,"Number of translated addresses = %d\n", numberOfTranslatedAddresses);
    ...
    fprintf(output_text,"Page Faults = %d\n", pageFaults);
    fprintf(output_text,"Page Fault Rate = %.3f\n", pfRate);
    fprintf(output_text,"TLB Hits = %d\n", TLBHits);
    fprintf(output_text,"TLB Hit Rate = %.3f\n", TLBRate);
